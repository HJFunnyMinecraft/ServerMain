...
void ReadConfig() {
	...
	while(reader != "#Config_End.") {
		...
		if(reader == "java=") {
			...
			MainConfig.JavaCommand = java.str();
		}
		if(reader == "timeout=") {
			int tmout;
			in >> tmout;
	...
	color(COLOR_SUCCESS);
	puts("[SUCC] 配置文件读取成功：");
	cout << "\tJavaCommand: " << MainConfig.JavaCommand << endl;
...
int main() {
	system("echo off");
	system("cls");
	setTitle("HECODE MCJAVA SERVER STARTER V1.1");
	color(COLOR_SUCCESS);
	puts("[SUCC] 软件启动成功！");
	puts(""); 
	color(COLOR_TIPS);
	puts("========== HECODE Minecraft Java 服务器启动器 ==========");
	puts("版本：V1.1");
	puts("构建日期：2022-11-23");
	puts("作者：CodeZhangBorui (HECODE)");
	puts("");
	puts("");
	while(1) {
		ReadConfig();
		...
		stringstream title;
		title 
			<< "HECODE MCJAVA SERVER STARTER V1.1 重启次数："
			<< RestartCount
			<< " 上次重启："
		...
		puts("");
		color(COLOR_DEFAULT);
		system(MainConfig.JavaCommand.c_str());
		puts("");
		color(COLOR_PROCESS);
		puts("检测到服务器终止，即将重启 ……");
		puts("若希望关闭服务器，请直接关闭此窗口。");
